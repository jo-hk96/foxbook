				...
				<c:forEach var="pagenum" begin="${searchDTO.pagination.startPage}" end="${searchDTO.pagination.endPage}" step="1">
					<c:choose>
						<c:when test = "${pagenum eq searchDTO.page}">
							<td class = "active"><a href="/BookList?page=${pagenum}&keyword=${searchDTO.keyword}">${pagenum}</a></td>		
						</c:when>
						<c:otherwise>
							<td><a href="/BookList?page=${pagenum}&keyword=${searchDTO.keyword}">${pagenum}</a></td>
						</c:otherwise>
					</c:choose>
				</c:forEach>
